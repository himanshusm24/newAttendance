...
const TaskHome = () => {
  ...
  const handleAllDashboardData = async () => {
    const dashboardData = await sendRequest(
      "get",
      `api/mobDashboard?user_id=${user_id}`,
      {},
      {
    ...
    setTotalDoneTask(data?.in_review_task_count);
              ...
              series={[
                {
                  data: [
                    ...
                    { id: 1, value: totalInProgressTask, label: "In Progress" },
                    { id: 2, value: totalInReviewTask, label: "In Review" },
